...
#pragma warning restore 0169, 0649
        ...
        public static bool IsValid(ByteArraySegment bas)
        {
            ...
            for(int i = 0; i<(EthernetFields.MacAddressLength * macRepetitions); i+=EthernetFields.MacAddressLength)
            {
                ...
                if(i == 0)
                {
                    ...
                    if(!SequenceEqual(buffer, syncSequence))
                        return false;
                }
                else
                {
                    ...
                    if (!SequenceEqual(buffer, destinationMAC))
                        return false;
                }
            }
            return true;
        ...
        private static bool SequenceEqual(byte[] b1, byte[] b2)
        {
            IEqualityComparer comparer = EqualityComparer<byte>.Default;
            IEnumerator enumerator = b1.GetEnumerator();
            IEnumerator enumerator2 = b2.GetEnumerator();
            {
